...
namespace Transejecutivos\Configuration;
...
class Services {
  ...
  public function setConfigFilePath($path) {
    $this->config = new \Phalcon\Config\Adapter\Ini($path);
  }
  ...
  private function setUrlManagerObject() {
    $this->urlManager = new \Transejecutivos\Misc\UrlManager($this->config);
  ...
  private function setDispatcher() {
    ...
    $di->set('dispatcher', function() use ($di) {
      ...
      $security = new \Transejecutivos\Plugins\Security();
      /**
...
//            return $flashMessage;
  ...
  public function setThemeData() {
    ...
    $this->di->set('theme', function() use ($config) {
      ...
      if (isset($config->theme->name)) {
        ...
        $theme->name = 'base';
        $theme->logo = '';
        $theme->title = 'Transportes Ejecutivos';
        $theme->subtitle = '';
        $theme->footer = '';
      }
      return $theme;
  ...
  private function setLogger() {
    ...
    $this->di->set('logger', function () use ($config) {
      return new \Phalcon\Logger\Adapter\File($config->log->folder);
    });
  }
  ...
  private function setVoltCompiler() {
    ...
    $di->setShared('volt', function($view, $di) use ($config){
      ...
      $volt->setOptions(array(
          "compileAlways" => true,
          "compiledPath" => $config->volt->compile_views_folder,
          'stat' => true
      ));
  ...
  private function setView() {
    ...
    $this->di->set('view', function() use ($config) {
      ...
      $view->setViewsDir($config->volt->views_folder);
